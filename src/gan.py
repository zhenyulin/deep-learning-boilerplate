...
with tf.Session() as s:
	...
	for epoch in range(EPOCHS):
		...
		for i in range(num_batches):
			...
			batch_images = batch[0].reshape((BATCH_SIZE, 784)) ### is it some shorthand?
			batch_images = batch_images*2-1 ### what this is doing?
			batch_seed = np.random.uniform(-1, 1, size=(BATCH_SIZE, 100))
			_ = s.run(D_trainer, feed_dict={real_images:batch_images, seed: batch_seed}) ### alternative function invoke?
